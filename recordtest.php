...
logit($logname, ' $_SESSION["recordteststep"]=' . $_SESSION['recordteststep']);

/*
 * record the test
 */
if ($_SESSION["recordteststep"] == 2) {
	// record test
...
if ($_SESSION["recordteststep"] == 1) {
	...
	foreach ($_SESSION["activestudents"] as $key => $value) {
		...
		if (isset($_POST["tested" . $value['stu_index']])) {
			...
			//echo $studentdata[$i]['srk_seq'] . ' ' . ($studentdata[$i]['srk_seq'] - $skip) . '<br>';
			...
			// locate the index in the array then subract 1 (or the amount of ranks skipped)
			...
			// calculate test fees
			// rank seq counts down from 11 to 1 then negative for black belt ranks, 0 isnt used
			//echo $studentdata[$i]['first_name'] . ' ' . $studentdata[$i]['srk_seq']	 . ' ' . $rankseq[$newrankindex] . '<Br>';
			$totalfee = 0;
			for ($j = $studentdata[$i]['srk_seq'] - 1; $j > $studentdata[$i]['srk_seq'] - $skip - 1; $j--) {
				//echo $j . ' ' . $studentdata[$i]['srk_seq'] . ' ' . $rankseq[$newrankindex] . ' '.$testfees[$j].'<Br>';
				if (isset($testfees[$j])) {
 ...
 */
if ($_SESSION["recordteststep"] == 0) {

	$_SESSION['artid'] = clean_user_input($_POST['artid']);
	$_SESSION['testdate'] = clean_user_input($_POST['testdate']);
	...
	try {
		...
		$_SESSION['recordtestartid'] = clean_user_input($_POST['artid']);
